...
public class CommentVerticle extends DynamoBusModBase {
   ...
   * </p>
   * AWS S3 spec: Entity tag that identifies the newly created object's data. Objects with
   * different object data will have different entity tags. The entity tag is an opaque string.
   * The entity tag may or may not be an MD5 digest of the object data. If the entity tag is not
   * an MD5 digest of the object data, it will contain one or more nonhexadecimal characters
   * and/or will consist of less than 32 or more than 32 hexadecimal digits.</p>
  ...
  private static String calculateChecksumForMultipartUpload(List<String> md5s) {
    ...
    return digest + (md5s.size() > 1 ? "-" + md5s.size() : "");
  }
    ...
    hasAccess(segment, message, parsedMessage.getJsonObject(), event -> {
      ...
      if (AccessCacheConstants.OK.getTitle().equals(accessValue)
        ...
        Item attachment = new Item()
            ...
            .withString("etag", DigestUtils.md5Hex(parsedMessage.getContentAsByteArray()));
