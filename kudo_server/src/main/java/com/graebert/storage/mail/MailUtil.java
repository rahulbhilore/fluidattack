...
public class MailUtil extends DynamoBusModBase {
  ...
  private void sendDemoMail(Message<JsonObject> message) {
    ...
    if (!config.getProperties().getSmtpEnableDemo()) {
      sendError(segment, message, "Demo emails aren't supported", 500);
      return;
    }
    ...
    if (!proName.contains("Kudo")) {
      ...
      sendError(
          segment, message, "Attempt to send notification from non-AK product - " + proName, 501);
      return;
    }
              ...
              handler.onComplete(sendEvent -> {
                ...
                if (sendEvent.succeeded()) {
                  ...
                  log.error(
                      ...
                      sendEvent.cause());
                  sendError(
                      segment, message, sendEvent.cause().getMessage(), 400, sendEvent.cause());
                }
              });
            } else {
  ...
  private void doNotifySaveFailed(Message<JsonObject> message) {
    ...
    handler.onComplete(event -> {
      if (event.succeeded()) {
        ...
        sendError(segment, message, event.cause().getMessage(), 400, event.cause());
      }
    });
  }
        ...
        eb_request(
            ...
            response -> {
              if (response.succeeded()) {
                ...
                String threadTitle = Utils.isStringNotNullOrEmpty(responseData.getString("title"))
                ...
                entries.forEach(event -> {
                  ...
                  // See mockups and https://graebert.atlassian.net/browse/XENON-37500
          ...
          if (attachmentResponse.succeeded()) {
            ...
            if (!tags.isEmpty()) {
              tagsText = tags.stream()
                  .map(tag -> ((JsonObject) tag).getString("text"))
    ...
    eb_request(
        ...
        response -> {
          if (response.succeeded()) {
            ...
            String metadataText = responseData.containsKey("text")
                    && Utils.isStringNotNullOrEmpty(responseData.getString("text"))
                ? responseData.getString("text")
                : "";
          ...
          if (checkResult.succeeded()) {
            ...
            String fileLink =
                instanceURL + "api/email/file/" + fileId + "?st=" + storageType.toUpperCase()
                    + (Utils.isStringNotNullOrEmpty(externalId) ? "&externalId=" + externalId : "")
  ...
  private void doNotifyResourceShare(Message<JsonObject> message) {
    ...
    String resourceThumbnail = json.getString("resourceThumbnail");
    String resourceName = json.getString(Field.RESOURCE_NAME.getName());
    String resourceParentId = json.getString("resourceParentId");
    OldResourceTypes resourceType =
        OldResourceTypes.getType(json.getString(Field.RESOURCE_TYPE.getName()));
  ...
  private void doNotifyShare(Message<JsonObject> message) {
    ...
    JsonArray viewers = json.getJsonArray("viewers");
    StorageType storageType =
        StorageType.getStorageType(json.getString(Field.STORAGE_TYPE.getName()));
  ...
  private void doConfirmRegistration(Message<JsonObject> message) {
    ...
    if (email == null || userId == null || nonce == null || username == null) {
      ...
      sendError(segment, message, "Email, userId, nonce or user name is not specified", 500);
      return;
    }
  ...
  private void doNotifyAdmins(Message<JsonObject> message) {
    ...
    if (withHash && nonce == null) {
      sendError(segment, message, "nonce is not specified", 500);
      return;
    }
    ...
    Item user = Users.getUserById(userId);
    if (user == null) {
      sendError(segment, message, Utils.getLocalizedString(message, "UserDoesNotExist"), 404);
      return;
    }
    if (withHash && !nonce.equals(user.getString(Field.NONCE.getName()))) {
      sendError(segment, message, Utils.getLocalizedString(message, "TheHashIsWrong"), 400);
      return;
    }
