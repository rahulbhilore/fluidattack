...
public class FontsVerticle extends DynamoBusModBase {
  ...
  private void doGetFonts(Message<JsonObject> message) {
    ...
    if (isCompanyFont) {
      ...
      if (company != null) {
        ...
        sendError(segment, message, 404, Utils.getLocalizedString(message, "CompanyNotFound"));
      }
    }
    ...
    try {
      ...
      if (company != null) {
        try {
          ...
          sendError(segment, message, 404, Utils.getLocalizedString(message, "FontNotFound"));
          return;
        }
      } else {
        sendError(segment, message, 404, Utils.getLocalizedString(message, "FontNotFound"));
  ...
  private String getUserId(Message<JsonObject> message, Entity segment, JsonObject json) {
    ...
    if (Utils.isStringNotNullOrEmpty(token) && Utils.isStringNotNullOrEmpty(fileId)) {
      ...
      try {
        ...
        sendError(segment, message, publicLinkException.toResponseObject(), 404, "PL1");
      }
    } else {
  ...
  private void doUploadFont(Message<JsonObject> message) {
      ...
      try {
        ...
        if (company != null) {
          try {
            ...
            sendError(
                segment, message, 400, Utils.getLocalizedString(message, "ErrorOnUploadFont"));
            return;
          }
        } else {
          sendError(segment, message, 404, Utils.getLocalizedString(message, "CompanyNotFound"));
        ...
        sendError(segment, message, 400, Utils.getLocalizedString(message, "ErrorOnUploadFont"));
        return;
      }
    } else {
      authorId = userId;
    ...
    try {
      ...
      sendError(segment, message, ple.toResponseObject(), 400);
      return;
    }
  ...
  private void doDeleteFont(Message<JsonObject> message) {
    ...
    try {
      ...
      if (deleteFont.getAuthorId().equals(userId)) {
        ...
        sendError(segment, message, 400, Utils.getLocalizedString(message, "NoUserFont"));
        return;
      }
    } catch (FontException e) {
      JsonObject company = parseCompany(json);
      if (company != null) {
        try {
          ...
          if (deleteFont.getAuthorId().equals(companyId) && isOrgAdmin) {
            ...
            sendError(
                segment, message, 400, Utils.getLocalizedString(message, "CantDeleteCompanyFont"));
            return;
          }
        } catch (FontException exception) {
          sendError(segment, message, 404, Utils.getLocalizedString(message, "FontNotFound"));
