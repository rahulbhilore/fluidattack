...
public class Templates extends Dataplane {
  ...
  public static Map<String, Object> getTemplate(String templateId, String userId) {
    ...
    return templates.stream()
        .filter(template -> template.get(Field.TEMPLATE_ID.getName()).equals(templateId))
        .findFirst()
        .orElse(null);
  }
  ...
  public static boolean deleteTemplate(String templateId, String id, TemplateType templateType) {
    ...
    if (item != null) {
      ...
      if (templates.removeIf(temp -> (temp.get(Field.TEMPLATE_ID.getName()).equals(templateId)))) {
  ...
  public static void saveTemplate(
    ...
    template.put(Field.TEMPLATE_ID.getName(), templateId);
  ...
  public static boolean updateTemplate(
    ...
    if (item != null) {
      List<Map<String, Object>> templates = item.getList(templatesField);
      templates.stream()
          .filter(temp -> temp.get(Field.TEMPLATE_ID.getName()).equals(templateId))
  ...
  public static void deleteTemplatesBulk(
    ...
    if (item != null) {
      List<Map<String, Object>> templates = item.getList(templatesField);
      boolean wasUpdated = templates.removeIf(
          template -> ids.contains((String) template.get(Field.TEMPLATE_ID.getName())));
      if (wasUpdated) {
        item.withList(templatesField, templates);
        putItem(item, DataEntityType.RESOURCES);
  ...
  public static void deleteTemplatesByType(
    ...
    if (item != null) {
      ...
      for (Map<String, Object> tmpl : templates) {
        templateIds.add((String) tmpl.get(Field.TEMPLATE_ID.getName()));
      }
