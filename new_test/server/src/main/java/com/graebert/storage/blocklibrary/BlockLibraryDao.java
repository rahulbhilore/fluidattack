...
public class BlockLibraryDao extends Dataplane {
  ...
  private static final String pkLockPrefix = pkMainPrefix.concat("LOCK").concat(hashSeparator);
  ...
  public static JsonObject itemToJson(Item item, boolean full) {
    ...
    String thumbnailName = ThumbnailsManager.getThumbnailName(
        Field.BL.getName(), thumbnailId, String.valueOf(item.getLong(Field.MODIFIED.getName())));
  ...
  private static String makeLockPkValue(String libraryId, String blockId) {
    return pkLockPrefix.concat(libraryId).concat(hashSeparator).concat(blockId);
  }

  public static void lockBlock(String libraryId, String blockId, String userId, String userEmail) {
    Item item = new Item()
        .withString(pk, makeLockPkValue(libraryId, blockId))
        .withString(sk, blockId)
        .withString(Field.LIB_ID.getName(), libraryId)
        .withString(Field.BLOCK_ID.getName(), blockId)
        .withString(Field.USER_ID.getName(), userId)
        .withString(Field.EMAIL.getName(), userEmail)
        .withLong(Field.CREATED.getName(), GMTHelper.utcCurrentTime())
        .withLong(
            Field.TTL.getName(), (GMTHelper.utcCurrentTime() + TimeUnit.HOURS.toMillis(1)) / 1000);

    putItem(blockLibraryTable, item, DataEntityType.RESOURCES);
  }

  public static Item findItemLock(String libraryId, String blockId) {
    final String pk = makeLockPkValue(libraryId, blockId);
    return getItemFromCache(
        blockLibraryTable, Dataplane.pk, pk, Dataplane.sk, blockId, DataEntityType.RESOURCES);
  }

  public static void unlockBlock(String libraryId, String blockId) {
    final String pk = makeLockPkValue(libraryId, blockId);
    deleteItem(
        blockLibraryTable, Dataplane.pk, pk, Dataplane.sk, blockId, DataEntityType.RESOURCES);
  }
