...
public abstract class GraphApi {
  ...
  protected final String userId, externalId, email, accountType;
  ...
  protected String siteUrl = emptyString;
  ...
  protected GraphApi(
    ...
    try {
      ...
      onedriveUser = ExternalAccounts.getExternalAccount(userId, externalId);

      if (Objects.isNull(onedriveUser)) {
        throw new Error("NoSuchExternalAccount");
      }
    ...
    checkRefreshToken();
    this.accountType = getUserAccountType(onedriveUser);
  }
    ...
    try {
      ...
      this.email = infoBody.containsKey("mail") && infoBody.getString("mail") != null
          ...
          : infoBody.containsKey("userPrincipalName")
              ...
              : emptyString;
      this.displayName = infoBody.containsKey(Field.DISPLAY_NAME.getName())
              && infoBody.getString(Field.DISPLAY_NAME.getName()) != null
          ? infoBody.getString(Field.DISPLAY_NAME.getName())
          : emptyString;
      ...
      this.accountType = getUserAccountType(externalAccount);
  ...
  public ObjectInfo getObjectInfo(
    ...
    String additionalParameters = emptyString;
    if (storageType.equals(StorageType.ONEDRIVEBUSINESS) && isBusinessAccount()) {
  ...
  public ObjectInfo getFileInfoByToken(String specificDriveId, String id) throws GraphException {
    ...
    String url = ids.getItemUrl(homeDrive)
        + (storageType.equals(StorageType.ONEDRIVEBUSINESS) && isBusinessAccount()
            ? "?$expand=versions($select=id)"
            : emptyString);
  ...
  public Permissions getPermissions(String driveId, JsonObject object) throws GraphException {
    checkRefreshToken();
    String url = GRAPH_URL + "/drive" + (driveId == null ? emptyString : "s/" + driveId) + "/items/"
  ...
  public Permissions getPermissions(String objectId) throws GraphException {
    ...
    // String url = GRAPH_URL + "/drive" + (ids.getDriveId() == null ? emptyString : "s/" +
    // ids.getDriveId
  ...
  public DriveInfo getMyDrive() throws GraphException {
    ...
    if (myDrives.getStatus() == HttpStatus.OK) {
      JsonArray allDrives = new JsonObject(myDrives.getBody()).getJsonArray("value");
      if (!allDrives.getJsonObject(0).containsKey("webUrl")) {
        return new DriveInfo(allDrives.getJsonObject(0));
      } else {
        Optional<JsonObject> optional = allDrives.stream()
            .filter(drive -> ((JsonObject) drive).getString("webUrl").endsWith("Documents"))
            .map(object -> (JsonObject) object)
            .findAny();
        if (optional.isPresent()) {
          return new DriveInfo(optional.get());
        }
  ...
  public CloneObjectResult cloneObject(String objectId, String name) throws GraphException {
    ...
    if (locationHeader.contains("/monitor/done/")) {
      ...
      return new CloneObjectResult(
          (ids.getDriveId() != null ? ids.getDriveId() + ":" : emptyString) + newObjectId,
          ids.getDriveId(),
    ...
    // copying may not be done yet
      ...
      return new CloneObjectResult(
          (ids.getDriveId() != null ? ids.getDriveId() + ":" : emptyString)
  ...
  private String getUserAccountType(Item onedriveUser) {
    if (Objects.nonNull(onedriveUser)) {
      if (!onedriveUser.hasAttribute("accountType")) {
        HttpResponse<String> driveResponse = AWSXRayUnirest.get(
                GRAPH_URL + "/me/drive", ".getDriveInfo")
            .header("Authorization", "Bearer " + this.accessToken)
            .asString();
        JsonObject body = new JsonObject(driveResponse.getBody());
        if (!driveResponse.isSuccess() || driveResponse.getStatus() != HttpStatus.OK) {
          throw new Error(getErrorMessageFromResponse(body));
        }
        if (body.containsKey("driveType")) {
          String driveType = body.getString("driveType");
          ExternalAccounts.updateUserAccountType(userId, externalId, driveType);
          return driveType;
        }
      } else {
        return onedriveUser.getString("accountType");
      }
    }
    return null;
  }
  ...
  private boolean isBusinessAccount() {
    return Utils.isStringNotNullOrEmpty(accountType) && accountType.equalsIgnoreCase("business");
  }
